...
/**
 * CaesarCipher class contains the following methods:
 * * encrypt() method uses Caesar cipher to encrypt input string using a chosen key value.
 * * encryptTwoKeys() method uses Caesar cipher to encrypt input string using chosen 2 key values.
 * 
 * 
 * @ Snehil S P
...
public class CaesarCipher {
    public String encrypt (String input, int key) {
        ...
        for (int i = 0; i < encrypted.length(); i++) {
            ...
            if (indexLower != -1) {
                // Get a character from the shiftedAlphabetLower that corresponds to the index
                char newCharLower = shiftedAlphabetLower.charAt(indexLower);
                // Replace the ith character of the encrypted string with newCharLower
